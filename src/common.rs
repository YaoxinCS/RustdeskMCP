...
pub const MCP_APP_NAME_ENV_KEY: &str = "RUSTDESK_MCP_APP_NAME";
...
pub fn pre_global_init_app_name() {
    let app_name = std::env::var(MCP_APP_NAME_ENV_KEY)
        .ok()
        ...
        .or_else(|| {
            std::env::current_exe()
                ...
                .filter(|name| name.eq_ignore_ascii_case("RustDeskMCP"))
        });
    let Some(app_name) = app_name else {
        return;
    };
    *hbb_common::config::APP_NAME.write().unwrap() = app_name.clone();
    if std::env::var(PORTABLE_APPNAME_RUNTIME_ENV_KEY)
        .ok()
        .filter(|value| !value.trim().is_empty())
        .is_none()
    {
        std::env::set_var(PORTABLE_APPNAME_RUNTIME_ENV_KEY, &app_name);
    }
}
