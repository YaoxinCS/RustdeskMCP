...
impl LoginConfigHandler {
    ...
    pub fn has_login_challenge(&self) -> bool {
        !self.hash.salt.is_empty() && !self.hash.challenge.is_empty()
    }

    pub fn queue_shared_password(&mut self, password: String) {
        self.password = Vec::new();
        self.password_source = Default::default();
        self.remember = false;
        self.shared_password = Some(password);
    }
